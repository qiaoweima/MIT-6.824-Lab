...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Term < rf.term {
		rf.Debug(dLog, "rececived term %d < currentTerm %d from S%d, reject AppendEntries", args.Term, rf.term, args.LeaderId)
		reply.Success = false
		return
	}
	...
	if rf.state == Candidate {
		...
		if args.PrevLogIndex > 0 {
			if prev := rf.GetLogAtIndex(args.PrevLogIndex); prev == nil || prev.Term != args.PrevLogTerm {
				...
				if prev != nil {
					confilctingIndex := prev.Index
					for i := prev.Index; i >= 1; i-- {
						if rf.log[i].Term == prev.Term {
							confilctingIndex = rf.log[i].Index
						} else {
							break
						}
					}
					if confilctingIndex > 1 {
						reply.ConfilctingIndex = rf.GetLogAtIndex(confilctingIndex - 1).Index
						reply.ConflictingTerm = rf.GetLogAtIndex(reply.ConfilctingIndex).Term
					} else {
						reply.ConfilctingIndex = 0
						reply.ConflictingTerm = 0
					}
				} else {
					reply.ConfilctingIndex = rf.LogTail().Index
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.votedFor == nil {
		...
		rf.Debug(dVote, "S%d RequestVote %v votedFor=S%d", args.CandidateId, *rf.votedFor)
	}
	if args.Term < rf.term {
...
func (rf *Raft) Sync(peer int, args *AppendEntriesArgs) {
	...
	if reply.Term > rf.term {
		rf.Debug(dHeartbeat, "return to Follower dute to reply.Term > rf.term")
		rf.state = Follower
		rf.resetTerm(reply.Term)
	} else {
		if reply.Success {
			...
			rf.nextIndex[peer] = reply.ConfilctingIndex + 1
		}
	}
} 
