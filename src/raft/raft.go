...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	reply.Term = rf.term
	if args.Term < rf.term {
		rf.Debug(dSnapshot, "received term < currentTerm, reject InstallSnapshot")
		rf.mu.Unlock()
		return
	}
	rf.lastHeartbeat = now
	rf.Debug(dSnapshot, "receive IntsallSnapshot from S%d args.term = %d LastIncludeIndex=%d LastIncludeTerm = %d", args.LeaderId, args.Term, args.LastIncludedIndex, args.LastIncludedTerm)
	if rf.state == Candidate && args.Term >= rf.term {
